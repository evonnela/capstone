...
const Book = ({ onPageChange, canNavigate }) => {
  ...
  const handlePageChange = (direction) => {
    ...
    if (!canNavigate && direction === 'next') return;
    ...
    const isLastPageOfChapter =
      newPage === pages[newChapter].length - 1 &&
      chapterIndices.includes(newChapter);

    onPageChange(newChapter, newPage, isLastPageOfChapter, chapterIndices);
  };
  ...
  return (
    <div className="book-container">
      ...
      {showSettings && (
        <FontSizeSettings fontSize={fontSize} changeFontSize={setFontSize} />
      )}
      ...
      <div className="page-navigation">
        ...
        <span className="chapter-info">
          {(chapters[currentChapter]?.title.trim() === ''
            ...
            : getChapterTitle(currentChapter)) || `Chapter ${currentChapter + 1}`}
        ...
        <button
          ...
          disabled={(currentChapter === chapters.length - 1 &&
            currentPage === (pages[currentChapter]?.length || 1) - 1) || !canNavigate}
        >
